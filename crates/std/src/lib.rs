...
use veryla_path::{PathError, PathSet};
...
fn std_dir() -> PathBuf {
    veryla_path::cache_path().join("std").join(STD_HASH)
}
...
pub fn expand() -> Result<(), PathError> {
    ...
    if !std_dir.exists() {
        ...
        let lock = veryla_path::lock_dir(&std_dir)?;
...
pub fn paths(base_dst: &Path) -> Result<Vec<PathSet>, PathError> {
    ...
    for src in &veryla_path::gather_files_with_extension(&std_dir, "veryla", false)? {
