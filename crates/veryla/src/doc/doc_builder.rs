...
use veryla_analyzer::symbol::{PowerDomain, ParameterKind, Symbol, SymbolKind};
use veryla_analyzer::symbol_table;
...
<tr>
    ...
    {{#if ../power_domains}}
    <td class="table_list_item"><span class="hljs-attribute">{{this.power_domain}}</span></td>
    {{/if}}
    <td class="table_list_item"><span class="hljs-type">{{this.typ}}</span></td>
    <td class="table_list_item">{{this.description}}</td>
</tr>
...
.table_list_item {
    ...
    fn build_entity(&self, name: &str, symbol: &Symbol) -> String {
        if let SymbolKind::Entity(property) = &symbol.kind {
            ...
            let power_domains: HashSet<_> = property
                .ports
                .iter()
                .filter_map(|x| {
                    if let PowerDomain::Explicit(_) = x.property().power_domain {
                        Some(x.property().power_domain.to_string())
                    } else {
                        None
                    }
                })
                .collect();
            let mut power_domains: Vec<_> = power_domains.into_iter().collect();
            power_domains.sort();
            ...
            let ports: Vec<_> = property
                ...
                .iter()
                .map(|x| {
                    let power_domain = if let PowerDomain::Explicit(_) = x.property().power_domain {
                        Some(x.property().power_domain.to_string())
                    } else {
                        None
                    };
