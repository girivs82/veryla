...
impl Lockfile {
    ...
    pub fn save<T: AsRef<Path>>(&mut self, path: T) -> Result<(), MetadataError> {
        ...
        text.push_str("# This file is automatically @generated by Veryla.\n");
    ...
    pub fn paths(&self, base_dst: &Path) -> Result<Vec<PathSet>, MetadataError> {
        ...
        for locks in self.lock_table.values() {
            for lock in locks {
                ...
                for src in &veryla_path::gather_files_with_extension(&path, "verylaa", false)? {
    ...
    fn resolve_path(url: &UrlPath) -> Result<PathBuf, MetadataError> {
        let resolve_dir = veryla_path::cache_path().join("resolve");
    ...
    fn resolve_version_from_latest(
        ...
        let lock = veryla_path::lock_dir("resolve")?;
    ...
    fn dependency_path(url: &UrlPath, revision: &str) -> Result<PathBuf, MetadataError> {
        let dependencies_dir = veryla_path::cache_path().join("dependencies");
