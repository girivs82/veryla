...
impl TokenRange {
    pub fn new(beg: &VerylaToken, end: &VerylaToken) -> Self {
        Self {
            beg: beg.token,
            end: end.token,
        }
...
impl From<&SequenceDeclaration> for TokenRange {
    fn from(value: &SequenceDeclaration) -> Self {
        let beg = value.sequence.sequence_token.token;
...
impl From<&VariableType> for TokenRange {
    fn from(value: &VariableType) -> Self {
        match value {
            VariableType::Power(x) => {
                let beg = x.power.power_token.token;
                let end = beg;
                TokenRange { beg, end }
            }
            VariableType::PowerPosedge(x) => {
                let beg = x.power_posedge.power_posedge_token.token;
                let end = beg;
                TokenRange { beg, end }
            }
            VariableType::PowerNegedge(x) => {
                let beg = x.power_negedge.power_negedge_token.token;
                let end = beg;
                TokenRange { beg, end }
            }
...
impl From<&CastingType> for TokenRange {
    fn from(value: &CastingType) -> Self {
        match value {
            ...
            CastingType::PowerPosedge(x) => {
                let beg = x.power_posedge.power_posedge_token.token;
                let end = beg;
                TokenRange { beg, end }
            }
            CastingType::PowerNegedge(x) => {
                let beg = x.power_negedge.power_negedge_token.token;
                let end = beg;
                TokenRange { beg, end }
            }
...
impl fmt::Display for VerylaToken {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl ScopedIdentifier {
    pub fn identifier(&self) -> &VerylaToken {
...
impl TryFrom<&StartToken> for VerylaToken {
    type Error = anyhow::Error;
...
macro_rules! token_with_comments {
    ($x:ident) => {
        paste! {
            impl TryFrom<&[<$x Token>]> for VerylaToken {
                type Error = anyhow::Error;
...
impl TryFrom<&EmbedContentToken> for VerylaToken {
    type Error = anyhow::Error;
